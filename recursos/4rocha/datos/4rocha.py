# -*- coding: utf-8 -*-

from gettext import gettext as _

NAME = _('Rocha')

STATES = [
    (_('Treinta y Tres'), 254, 283, 86, 0),
    (_('Lavalleja'), 253, 135, 289, 0),
    (_('Maldonado'), 252, 100, 800, 0),
    (_('Rocha'), 251, 394, 408, 0),
    (_('Brasil'), 250, 738, 182, -90)
]

CITIES = [
    (_('Rocha'), 303, 721, 1, 4, -16),
    (_('Cebollatí'), 543, 96, 2, 20, 12),
    (_('San Luis al Medio'), 570, 275, 2, -10, -16),
    (_('Lascano'), 347, 321, 2, 0, -14),
    (_('18 de Julio'), 638, 312, 2, -40, -14),
    (_('La Coronilla'), 652, 420, 2, -40, -14),
    (_('Velázquez'), 308, 530, 2, 0, -14),
    (_('Castillos'), 510, 580, 2, 0, -14),
    (_('La Paloma'), 375, 798, 2, 22, 18),
    (_('La Pedrera'), 388, 765, 2, 48, 8),
    (_('Cabo Polonio'), 529, 670, 2, 62, 0),
    (_('Barra de Valizas'), 528, 639, 2, 72, -2),
    (_('Aguas Dulces'), 529, 615, 2, 62, 0),
    (_('Punta del Diablo'), 634, 504, 2, 72, 4),
    (_('Pque. Santa Teresa'), 645, 447, 2, -65, -14),
    (_('Barra del Chuy'), 705, 352, 2, -64, 2),
    (_('Chuy'), 678, 317, 2, -10, 12),
    (_('19 de Abril'), 405, 680, 2, 0, -12)
]

RIVERS = [
    (_('Océano Atlántico'), 254, 627, 754, 45),
    (_('Río Cebollatí'), 253, 416, 164, 40),
    (_('Río San Luis'), 252, 586, 327, 45),
    (_('A. de las Pelotas'), 251, 492, 199, 20),
    (_('A. de la India Muerta'), 250, 357, 369, 60),
    (_('A. S. de la Paloma'), 249, 295, 533, 70),
    (_('A. Sarandí de la I. M.'), 248, 392, 510, -55),
    (_('A. Sauce'), 247, 439, 470, -30),
    (_('A. Sauce Caído'), 246, 451, 385, -50),
    (_('A. San Miguel'), 245, 606, 345, 45),
    (_('A. S. del Peñon'), 244, 502, 424, 90),
    (_('A. del Sarandí'), 243, 605, 414, 35),
    (_('A. del Chuy'), 242, 711, 326, -45),
    (_('A. del Peñon'), 241, 521, 508, -10),
    (_('A. del Sauce'), 240, 535, 543, 0),
    (_('A. Castillos'), 239, 459, 564, 90),
    (_('A. Don Carlos'), 238, 367, 691, -30),
    (_('A. del Chafalote'), 237, 410, 645, -75),
    (_('A. Garzón'), 236, 203, 761, 90),
    (_('A. Carpinchos'), 235, 265, 718, -50),
    (_('A. del Oeste'), 234, 261, 645, 90),
    (_('A. de los Rocha'), 233, 292, 643, 70),
    (_('A. del Alférez'), 231, 222, 508, 90),
    (_('A. Aiguá'), 230, 265, 336, 70),
    (_('Lag. Merin'), 228, 670, 42, 45),
    (_('Lag. Negra'), 227, 583, 478, 0),
    (_('Lag. Castillos'), 226, 481, 626, 0),
    (_('Lag. de Rocha'), 225, 326, 783, 0)
]

CUCHILLAS = [
    (_('Cuchilla de las Averías'), 254, 361, 346, 55),
    (_('Cuchilla de la Carbonera'), 253, 506, 481, 35),
    (_('Cuchilla de la India Muerta'), 252, 409, 584, -60)
]

HILLS = [
    (_('Co. de Aguirre'), 363, 649, 5, 0, 14)
]

ROUTES = [
    (_('Route %s') % 9, 250, 422, 626, 55),
    (_('Route %s') % 10, 249, 441, 698, 30),
    (_('Route %s') % 109, 248, 250, 699, -30),
    (_('Route %s') % 15, 247, 291, 608, 90),
    (_('Route %s') % 13, 246, 414, 498, 0),
    (_('Route %s') % 16, 245, 528, 500, 80),
    (_('Route %s') % 14, 244, 453, 374, -30),
    (_('Route %s') % 19, 243, 499, 276, -20)
]
